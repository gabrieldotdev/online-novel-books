    ...
    <Card className="border-none shadow-none bg-white">
      ...
      <CardContent className="space-y-3 mt-3">
        {Array.from({ length: 7 }).map((_, idx) => (
          <Link key={idx} href="/profile/d.wxs" className="flex items-center justify-between w-full space-x-3">
            <div className="flex flex-col space-y-1">
              <CardTitle>D.Wxs</CardTitle>
              <CardDescription className="line-clamp-1">Chưa có thông tin</CardDescription>
            </div>
            <UserAvatar src="/assets/images/600.webp" fallback={idx + 1} className="w-10 h-10 text-base" />
          </Link>
        ))}
      </CardContent>
    </Card>
